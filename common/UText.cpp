#include "UText.h"

// AppearanceEdit.exe: 00461428
static const byte __lower_map[256] = {
    };

// AppearanceEdit.exe: 00461528
static const byte __upper_map[256] = {
    };

// AppearanceEdit.exe: 004438f0
bool UText::Equal(const char *inTextA, const char *inTextB, uint inSize)
{
	short *psVar1;
	short *psVar2;
	uint uVar3;

	if ((inSize == 0) || (inTextA == inTextB))
	{
		return true;
	}
	if ((inTextA != NULL) && (inTextB != NULL))
	{
		uVar3 = -(longlong)inTextA & 7;
		if (uVar3 != (-(longlong)inTextB & 7U))
		{
			uVar3 = 0;
			if (inSize != 0)
			{
				do
				{
					if (inTextA[uVar3] != inTextB[uVar3])
					{
						return false;
					}
					uVar3 = uVar3 + 1;
				} while (uVar3 < inSize);
			}
			return true;
		}
		if (inSize < uVar3)
		{
			uVar3 = inSize;
		}
		if (uVar3 != 0)
		{
			if (*inTextA != *inTextB)
			{
				return false;
			}
			if (1 < uVar3)
			{
				if (inTextA[1] != inTextB[1])
				{
					return false;
				}
				if ((2 < uVar3) && (inTextA[2] != inTextB[2]))
				{
					return false;
				}
			}
			inTextA = inTextA + uVar3;
			inTextB = inTextB + uVar3;
			inSize = inSize - uVar3;
		}
		uVar3 = 0;
		if (inSize >> 2 != 0)
		{
			do
			{
				if (*(int *)(inTextA + uVar3 * 4) != *(int *)(inTextB + uVar3 * 4))
				{
					return false;
				}
				uVar3 = uVar3 + 1;
			} while (uVar3 < inSize >> 2);
		}
		uVar3 = inSize & 3;
		if (uVar3 != 0)
		{
			psVar2 = (short *)(inTextA + (inSize & 0xfffffffc));
			psVar1 = (short *)(inTextB + (inSize & 0xfffffffc));
			if (uVar3 == 3)
			{
				if (*psVar2 != *psVar1)
				{
					return false;
				}
				if ((char)psVar2[1] != (char)psVar1[1])
				{
					return false;
				}
			}
			else if (uVar3 == 2)
			{
				if (*psVar2 != *psVar1)
				{
					return false;
				}
			}
			else if ((char)*psVar2 != (char)*psVar1)
			{
				return false;
			}
		}
		return true;
	}
	return false;
}

// AppearanceEdit.exe: 004335e0
// KDXClient.exe: 004575a0
bool UText::IsHex(uint inChar)
{
	bool bVar1;

	bVar1 = true;
	if ((((inChar < 0x30) || (0x39 < inChar)) && ((inChar < 0x41 || (0x46 < inChar)))) &&
	    ((inChar < 0x61 || (0x66 < inChar))))
	{
		bVar1 = false;
	}
	return bVar1;
}

// AppearanceEdit.exe: 00433650
uint UText::ToLower(uint inChar)
{
	if (inChar < 0x100)
	{
		inChar = (uint)__lower_map[inChar];
	}
	return inChar;
}

// AppearanceEdit.exe: 00433630
uint UText::ToUpper(uint inChar)
{
	if (inChar < 0x100)
	{
		inChar = (uint)__upper_map[inChar];
	}
	return inChar;
}
